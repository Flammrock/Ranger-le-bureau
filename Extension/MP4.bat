...
:: Vérification
for /f "tokens=* delims=" %%a in ('type %currentpath2%extension.ini^| Find /i "MP4"') do echo %%a > %currentpath2%001.txt
...
if exist "%currentpath%*.MP4" MD "%currentpath%Video-rangés/MP4"
:: ####### DEPLACEMENTS FICHIERS
:: Copie des fichiers selon une extention précise
FOR %%f IN ("%currentpath%*.MP4" ) DO ( 
move %%f "%currentpath%Video-rangés/MP4"
