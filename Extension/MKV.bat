...
:: Vérification
for /f "tokens=* delims=" %%a in ('type %currentpath2%extension.ini^| Find /i "MKV"') do echo %%a > %currentpath2%001.txt
...
if exist "%currentpath%*.MKV" MD "%currentpath%Video-rangés/MKV"
:: ####### DEPLACEMENTS FICHIERS
:: Copie des fichiers selon une extention précise
FOR %%f IN ("%currentpath%*.MKV" ) DO ( 
move %%f "%currentpath%Video-rangés/MKV"
