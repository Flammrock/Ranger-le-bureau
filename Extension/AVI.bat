...
:: Vérification
for /f "tokens=* delims=" %%a in ('type %currentpath2%extension.ini^| Find /i "AVI"') do echo %%a > %currentpath2%001.txt
...
if exist "%currentpath%*.AVI" MD "%currentpath%Video-rangés/AVI"
:: ####### DEPLACEMENTS FICHIERS
:: Copie des fichiers selon une extention précise
FOR %%f IN ("%currentpath%*.AVI" ) DO ( 
move %%f "%currentpath%Video-rangés/AVI"
