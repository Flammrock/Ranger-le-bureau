...
:: Vérification
for /f "tokens=* delims=" %%a in ('type %currentpath2%extension.ini^| Find /i "JPG"') do echo %%a > %currentpath2%001.txt
...
if exist "%currentpath%*.JPG" MD "%currentpath%images-rangées/JPG"
:: ####### DEPLACEMENTS FICHIERS
:: Copie des fichiers selon une extention précise
FOR %%f IN ("%currentpath%*.JPG" ) DO ( 
move %%f "%currentpath%images-rangées/JPG"
